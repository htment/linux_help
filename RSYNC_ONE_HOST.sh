...
create_index() {
    ...
    find "$SOURCE_DIR" -type f | sed '/^$/d' | sort > "$SOURCE_INDEX_FILE"
    if [ $? -ne 0 ]; then
...
copy_single_file() {
    ...
    for attempt in $(seq 1 $MAX_ATTEMPTS); do
        log "Копирование файла '$source_file', попытка $attempt из $MAX_ATTEMPTS"
        rsync $RSYNC_OPTS "$source_file" "$dest_file" >> "$LOGFILE" 2>&1
        rsync_exit_code=$?

        if [ $rsync_exit_code -eq 0 ]; then
            log "Файл '$source_file' успешно скопирован."
            echo "$source_file" >> "$SUCCESS_LIST"
            return 0
        else
            log "Ошибка копирования файла '$source_file' (exit code: $rsync_exit_code). Ожидание $RETRY_DELAY секунд..."
            sleep $RETRY_DELAY
        fi
    done

    log "Не удалось скопировать файл '$source_file' после $MAX_ATTEMPTS попыток."
    echo "$source_file" >> "$FAILED_LIST"
    return 1
}
...
while IFS= read -r source_file; do
    # Получаем относительный путь (относительно SOURCE_DIR)
    ...
    if ! copy_single_file "$source_file" "$dest_file"; then
...
log "Успешно скопировано: $(wc -l < "$SUCCESS_LIST") файлов"
log "Не удалось скопировать: $(wc -l < "$FAILED_LIST") файлов"
